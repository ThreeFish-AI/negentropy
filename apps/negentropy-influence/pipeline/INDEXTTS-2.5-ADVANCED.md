...
> 即实际权重，各维「性价比」相同，取舍只能靠试听。
> 反向证据：bias 表里被下调的恰是 `surprised`/`calm`，说明上游认为它们在名义权重下
> **偏强**需要压——我们拿到的是未经衰减的版本。
...
### 6.6 内容侧的噪声地板：韵律 A/B 的最小可归因效应量 ≈ 21%（2026-09-02）
...
| 量 | 中位 | 逐句范围 |
| --- | --- | --- |
| `\|ΔIQR_rel\|`（起伏÷音高中位） | **10.4%** | −10.4% … +21.3% |

⇒ **任何 <10% 的韵律「改善」都无法与换一条 take 区分**；按「效应量 ≥ 2×噪声」定门，
韵律类参数 A/B 的**最小可归因效应量是 ≈21%**。这一条直接废掉了一批看起来诱人的小改动：
...
2. **身份护栏用中位，不用逐句最大值**。§3.1 的 alpha→音高标定曲线（169 / 188 / 199–223 Hz）
   ...
   改回中位口径后三者反而**低于**基线。
3. **小样本下看符号一致性，不只看中位**。n=7 时中位的标准误约 噪声/√7，故「中位涨了多少」
   很不稳；`胜出 k/n` 的符号检验更可信（7/7 → p=0.008，5/7 → p=0.23）。本轮出现过
   「中位最高（+18.1%）但只有 5/7 一致、靠两句离群值撑起来」的臂，若只看中位就会选错。
