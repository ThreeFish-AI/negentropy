...
| **3D 表意强化** | ✅ 新增 `video/src/components/solids-3d.tsx`（332 行原语层，seeded 档；零 motion hook、运动量全部 prop 注入）。落地两处：**5-D「壳」**——p5-17 是全片唯一把整套结构命名为**三维实体名词**的句子，原实现只是 3px 边框 div（读作「框」）；改为四层实心板嵌套（层序即 z 序：循环最内、插口最外），终端沉在井底，「里面」成为几何事实。**4-C「插座插头」**——插座的定义就是「凹进去的洞」，原 22×22 方框只是**画在墙上的符号**、「插入」动作在平面上根本不存在；3D 咬合的侧棱遮挡是「它真的进去了」的唯一无歧义信号。`PlateSlab3D` 收敛到 `Slab3D`，**同帧 PNG md5 逐字节相同**（并删除对 basic 材质空操作的 `ambientLight`）。时序**零改动**：四层合拢开在既有 26 帧 shell 进度的子窗口（frozen `win()`）、插头直接吃既有 `useSpring('snap')` ⇒ 无新 hook、无新 `@动词`、总帧数由 manifest 结构性推导。三轮实景抽帧修复见 [ISSUE-180](../../../../docs/.agents/issue.md)（线框堆 / 面色与 bg 同色 / `zIndex:-1` 沉底） |
