...
> **改造状态（2026-09-06 内容勘误 + 3D 表意强化）**：官方文档复核修正 3 句（第五样勘误 / hook 事件数改抗漂移口径 / 取数日期）并补录轨 C 六条官方信源；
> 5-D「壳」与 4-C「插座插头」3D 化（新增 `components/solids-3d.tsx` 原语层，`PlateSlab3D` 收敛且逐字节零变化）。
> **前次（2026-09-03 运动层重制）**：全片动效收敛至 frozen 运动层（`video/src/motion/`，跨集共享「怎么动」）；HarnessStack 系列装置落地（P0 开场落板+顶边常驻条+P6 收尾）；配音全量重合成（音频版本库入库，改稿只重配变更句）。
...
| **终渲交付（2026-09-06 本轮）** | ✅ `out/final.mp4` **13:59.90**（839.90 秒 / **25197 帧**，`total_duration_in_frames` 复算恒等）· 1920×1080 @30fps · h264/yuvj420p + aac 189kb/s · **42.2 MB**；`cover.png` = 标题卡帧 1072（画面已验第五辐条为「循环」核色）<br>时长较前版 13:58.80 仅 **+1.10 秒**（改稿净增 10 字），双口径均落窗 [13.0, 14.6]<br>**终渲全分辨率复检 FAIL 0 · WARN 0**（七幕 + 末 6 句；草渲期唯一的 p2-07/p2-10 哈希 WARN 在全分辨率下消失，佐证其为均值哈希假阳性）<br>草渲期门：七幕 FAIL 0 WARN 1 · `--last-n 6` FAIL 0 WARN 0 · **`--beat-heads 4`（39 镜 ×4 帧）FAIL 0 WARN 0**<br>⚠️ 过程教训：两个渲染进程并发写同一 `draft.mp4` 会产出**索引损坏**的文件（seek 全部落回第 0 帧、`dup=11768`），据此抽出的 52 条 WARN 全是伪信号——**抽帧前先验产物 seek 正常**（同一文件取两个时间戳应得不同 md5） |
| 前版终渲 | `out/final.mp4` **13:58.80**（838.8 秒 / 25164 帧，与草渲同 manifest 同帧数）· 1920×1080 @30fps · 43.6 MB；`cover.png` = 标题卡帧 1031<br>时长**可复算**：`total_duration_in_frames(narration.json, timing.json)` = 25164 帧 @30fps = 838.8s<br>终渲全分辨率七幕抽帧 **FAIL 0 · WARN 1**（p0-09/10 帧指纹相同 = 0-D 标题卡跨两句静止，刻意）<br>**2026-08-22 画面优化版**（narration 与配音零改动，纯 tier-i 视觉增强 + `--update` 走查）：P6 三挂件卡物理咬合上环 + 信源卡增站点/双钉/实测口径诚实行；P0 走秒芯片（凝住即停）、轮次钢印 01/02/03、载荷箭头（命令/输出）；P5 环第 6 次出场 + 20–28 行实测基准带；1-D 诚实角注（站点教学版仍按停止标记）；3-C 三判定小抄（allow/ask/deny 各带载荷）；`📣`/`⚠` emoji 全部替换为绘制图形<br>**同日评审四修**（逐帧抽帧实证，见下方「评审修复」）：6-A 挂件卡文字朝向与落位、6-A 挂件卡**入场瞬态**压字幕安全带、5-B 基准带锚点与标注避让、3-C 小抄不再遮挡闸门名。末次重渲 473s，七幕抽帧复检 **FAIL 0 · WARN 1**（仍为 0-D 刻意静止） |
| 交付件 | ✅ `final.mp4` + `captions.srt` + `captions.vtt` + `cover.png`（标题卡帧，1920×1080） |
| 内容勘误（2026-09-06） | ✅ 官方文档逐条复核（glossary / how-claude-code-works / hooks / permissions / auto-mode / harnessing 六页，取数 2026-09-06）修正 **3 句**：`p0-06` 第五样「护栏」→「**把这些串起来的循环**」（官方原文 "the loop that chains actions together"；**原稿此断言在事实源中无锚点**，见 [ISSUE-178](../../../../docs/.agents/issue.md)）· `p4-23`「三十一个」→「**三十多个**」（实测已漂移到 33，绝对数属活数据，[ISSUE-179](../../../../docs/.agents/issue.md)）· `p6-06` 取数日期。字数 3979 → **3989**（+10），重合成仅 3 句（其余 164 句版本库回收）；连带改 P0-C 第五辐条（染 `core`，成为 P1 环诞生的引信）、4-F 计数 31→33、P0/P6 取数日期 |
| 信源台账补录 | ✅ 轨 C · 官方文档六条入 `sources.toml`（`cc-glossary` / `cc-how-it-works` / `cc-hooks` / `cc-permissions` / `cc-auto-mode` / `cc-harness-blog`），`verify` 受检面 **12 → 18 条 FAIL 0**；`source-notes.md` 新增轨 C 与 **§十 官方断言表**（12 条断言逐条附原文）。⚠️ 补录前 verify 报「受检 12 · FAIL 0」全绿——**门是绿的，只因为出问题的六页不在受检集里** |
| **3D 表意强化** | ✅ 新增 `video/src/components/solids-3d.tsx`（298 行原语层，seeded 档；零 motion hook、运动量全部 prop 注入）。落地两处：**5-D「壳」**——p5-17 是全片唯一把整套结构命名为**三维实体名词**的句子，原实现只是 3px 边框 div（读作「框」）；改为四层实心板嵌套（层序即 z 序：循环最内、插口最外），终端沉在井底，「里面」成为几何事实。**4-C「插座插头」**——插座的定义就是「凹进去的洞」，原 22×22 方框只是**画在墙上的符号**、「插入」动作在平面上根本不存在；3D 咬合的侧棱遮挡是「它真的进去了」的唯一无歧义信号。`PlateSlab3D` 收敛到 `Slab3D`，**同帧 PNG md5 逐字节相同**（并删除对 basic 材质空操作的 `ambientLight`）。时序**零改动**：四层合拢开在既有 26 帧 shell 进度的子窗口（frozen `win()`）、插头直接吃既有 `useSpring('snap')` ⇒ 无新 hook、无新 `@动词`、总帧数由 manifest 结构性推导。三轮实景抽帧修复见 [ISSUE-180](../../../../docs/.agents/issue.md)（线框堆 / 面色与 bg 同色 / `zIndex:-1` 沉底）。<br>⚠️ **本轮评审后修（2026-09-06 晚）**：5-D 四层的 `depth` 映射与声明的「自内向外」相反，成片里 core 色的「循环」被渲染成**最外圈**（见 [ISSUE-182](../../../../docs/.agents/issue.md)）；同时把壳画布按投影实测加高（原余量漏算合拢期的 z 后推量，最外层底缘被画布下沿直切 635px）。**修后 `out/final.mp4` 已过期，5-D 段需重渲**；4-C 帧 17392 修前后逐像素相同（`skin.face` 本就是空操作）。 |
