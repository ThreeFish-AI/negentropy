      ...
      <svg width={1920} height={1080} style={{position: 'absolute', inset: 0}}>
        <g transform={`translate(${CX} ${CY}) scale(${breathe})`}>
          ...
          {attach.map((a, i) => {
            ...
            // ⚠️ 勿再放大 dist：卡片恒正立，轴对齐半宽 84 / 半高 34 在 R+96 下最近
            // 角到环心 ≈ 216–225 > 环线外沿 205，已留 11–20px 净空；挂脚末端夹爪
            // 咬在环线上是**设计意图**，不是重叠缺陷。2026-08 帧级复查曾把这段夹爪
            // 误判为「卡片压环」并把 dist 提到 R+190，结果 75° 的「闸门」卡下沿掉到
            // y≈960、探进字幕安全带（SAFE_TOP_Y=920）——按下葫芦浮起瓢，已回退。
            const dist = R + 96;
