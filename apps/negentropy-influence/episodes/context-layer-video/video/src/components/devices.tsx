/** 本集专用视觉装置库（v3）。
 *
 *  统一空间隐喻 = **受治理大厦五层纵剖面**：五个正交层各占一个固定部位、严格单射、
 *  全片不换位（放/找/养/信/用）。角落常驻五格承重柱 HUD，讲完一层点亮一根柱子。
 *
...
const GRADE: Record<EvidenceGrade, {t: string; c: string}> = {
  lab: {t: '【一】本仓原型实测输出', c: theme.activate},
  official: {t: '【二】官方机制转述', c: theme.dim},
  vendor: {t: '【三】厂商自报基准', c: theme.blueprint},
  thirdparty: {t: '【四】第三方独立复现', c: theme.grown},
};
          ...
          <div key={n} style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5}}>
            <div
              style={{
                ...
                height: 40 + 14 * p,
                borderRadius: 3,
                background: on ? theme.blueprint : `${theme.panelBorder}`,
                boxShadow: on ? `0 0 ${10 * p}px ${theme.blueprint}88` : 'none',
...
/** 母图部位名：五正交层严格单射，全片不换位（放/找/养/信/用） */
export type BuildingFocus = 'objects' | 'catalog' | 'enrich' | 'govern' | 'activate';

/** 大厦纵剖面母图：五个正交层固定落位，`focus` 指定本幕高亮哪一层 */
export const BuildingSection: React.FC<{
  ...
  const floors: {k: NonNullable<typeof focus>; label: string; c: string}[] = [
    {k: 'objects', label: '顶层规章室 · 对象层', c: theme.blueprint},
    {k: 'catalog', label: '索引总账层 · 目录层', c: theme.blueprint},
    {k: 'enrich', label: '双轨编纂层 · 富化层', c: theme.grown},
    {k: 'govern', label: '风控承重层 · 治理层', c: theme.activate},
    {k: 'activate', label: '一层前台与插座 · 激活层', c: theme.activate},
  ];
  const H = 78;
  return (
    <div
...
/** 层推近：母图定位该层所在部位 + 右栏逐句落位的卡片。
 *
 *  讲到哪一层，先把摄影机推到它在大厦里的固定部位（P2–P5 各幕定位各自的层）。 */
