...
| 0-E | p0-11..13b | **两条老路**：左船雾中盲航（引航青雾）翻沉 / 右船超载（警示金）压舱进水；旁挂数字翻牌「20×3000≈60000」 ·**archify full**：two-old-roads 章 `or-two`（p0-11）+ `or-guess`（p0-12）+ `or-stuff`（p0-12a）+ `or-ledger`（p0-13） | 双船对照 useProgress 同步下沉；翻牌 useCount；`@progress` `@count` |

## P1 一个文件夹的标准（p1-01..12g，镜 1-A..1-G）
...
| 1-C | p1-07..11 | **正文与附件**：正文区自由书写动画 + 三层附件抽屉（scripts/references/assets）推入箱体；「规范不管」印章 ·**archify full**：box-anatomy 章 `ba-body`（p1-07）+ `ba-extra`（p1-08a） | 抽屉 useStagger 推入；印章 useImpulse；`@stagger` `@impulse` |
