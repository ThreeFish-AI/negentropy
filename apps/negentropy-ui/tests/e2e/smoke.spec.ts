...
test("Knowledge Pipelines 状态标签视觉一致性", async ({ page }) => {
  await mockAuthenticatedUser(page);
  ...
  for (const theme of ["light", "dark"] as const) {
    ...
    expect(pipelinesRunning.badgeClassName).toContain("inline-flex");
    expect(pipelinesRunning.badgeClassName).toContain("gap-2");
    expect(pipelinesRunning.dotClassName).toContain("animate-pulse");
    expect(pipelinesRunning.textClassName).toContain("text-amber-600");
    expect(pipelinesCompleted.textClassName).toContain("text-emerald-600");
    expect(pipelinesFailed.textClassName).toContain("text-rose-600");
  }
});

test("聊天流式 Markdown 表格与换行在 hydration 后无需刷新即可稳定收敛", async ({ page }) => {
  ...
  await expect(page.getByRole("columnheader", { name: "项目" })).toHaveCount(1);
  await expect(page.getByRole("cell", { name: "首行" })).toHaveCount(1);
  await expect(page.getByRole("cell", { name: "次行" })).toHaveCount(1);
  ...
  await expect(page.getByRole("heading", { level: 2, name: "分析" }).locator("..")).toHaveCount(1);
