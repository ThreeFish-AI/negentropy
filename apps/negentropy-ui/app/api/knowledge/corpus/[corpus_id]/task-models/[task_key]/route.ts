...
export async function PUT(
  request: Request,
  { params }: { params: Promise<{ corpus_id: string; task_key: string }> },
) {
  const { corpus_id, task_key } = await params;
  ...
  const upstreamUrl = new URL(
    `/knowledge/corpus/${encodeURIComponent(corpus_id)}/task-models/${encodeURIComponent(task_key)}`,
    baseUrl,
  );
