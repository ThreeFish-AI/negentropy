...
export default function KnowledgePipelinesPage() {
  const [pipelinesData, setPipelinesData] = useState<KnowledgePipelinesData | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const dashData = await fetchPipelinesData(APP_NAME);
        if (active) setPipelinesData(dashData);
      } catch (err) {
        if (active) setError(String(err));
      }
    })();
  ...
  const metrics = useMemo(() => {
    ...
    return [
      { label: "Corpus", value: pipelinesData.corpus_count },
      { label: "Knowledge", value: pipelinesData.knowledge_count },
      {
        label: "Last Build",
        value: pipelinesData.last_build_at ? (
          <span title={pipelinesData.last_build_at}>
            {new Intl.DateTimeFormat("zh-CN", {
              ...
              hour: "2-digit",
              minute: "2-digit",
            }).format(new Date(pipelinesData.last_build_at))}
          </span>
        ) : (
          "-"
        ),
