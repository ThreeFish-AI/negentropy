...
export function KgRunDetailPanel({ run }: { run: KgPipelineRun }) {
  return (
    <div className="mt-3 min-w-0 space-y-3 text-xs text-text-secondary">
      ...
      {/* 实体/关系统计 */}
      <div className="rounded-lg border border-border bg-muted p-3">
        <p className="text-caption uppercase tracking-overline text-text-muted">Statistics</p>
        <div className="mt-2 grid grid-cols-2 gap-3">
          <div>
            <p className="text-micro text-text-muted">实体数</p>
            <p className="text-sm font-semibold text-text-secondary tabular-nums">
              {run.entity_count}
            </p>
          </div>
          <div>
            <p className="text-micro text-text-muted">关系数</p>
            <p className="text-sm font-semibold text-text-secondary tabular-nums">
              {run.relation_count}
            </p>
          </div>
      ...
      {run.stages && Object.keys(run.stages).length > 0 && (
        <div className="rounded-lg border border-border bg-muted p-3">
          <p className="text-caption uppercase tracking-overline text-text-muted">Phases</p>
      ...
      {run.extractor_config && Object.keys(run.extractor_config).length > 0 && (
        <div className="min-w-0">
          <p className="text-caption uppercase tracking-overline text-text-muted">Extractor Config</p>
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 max-h-32 space-y-1.5 overflow-auto">
              {realWarnings.map((w, i) => (
                <div
                  key={i}
                  className="rounded border border-amber-200 bg-amber-50 p-2 text-caption text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
                >
                  {JSON.stringify(w)}
                </div>
              ))}
      ...
      {run.error_message && (
        <div className="min-w-0">
          <p className="text-caption uppercase tracking-overline text-text-muted">Error</p>
