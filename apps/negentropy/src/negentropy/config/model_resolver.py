...
async def resolve_subagent_model_name(agent_name: str | None) -> str | None:
    ...
    - 60s TTL 缓存，键为 ``subagent:<agent_name>``；SubAgent PATCH/DELETE 端点
      调用 ``invalidate_cache(prefix="subagent:")`` 或 ``invalidate_cache("subagent:<name>")``
      实现强一致。
    """
    if not agent_name:
        return None
    ...
    entry = _cache.get(cache_key)
    if entry is not None:
        name, kwargs, ts = entry  # 复用三元组；kwargs 占位 dict 不用
        if now - ts < _CACHE_TTL:
            # name 存储即是 model 字符串或空串（""=显式无配置）
            return name or None

    try:
        model_value = await _load_subagent_model(agent_name)
    except Exception:
        from negentropy.logging import get_logger

        get_logger("negentropy.config.model_resolver").warning(
            "subagent_model_load_failed",
            agent_name=agent_name,
            exc_info=True,
        )
        return None

    # 空串占位：使缓存同样适用于「未配置」场景，避免重复 DB 查询
    _cache[cache_key] = (model_value or "", {}, now)
...
async def _load_subagent_model(agent_name: str) -> str | None:
    """从 ``sub_agents`` 表读取 ``model`` 字段字符串；不存在或空返回 None。"""
    from sqlalchemy import select
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(SubAgent.model, SubAgent.is_enabled).where(SubAgent.name == agent_name).limit(1)
        )
        row = result.first()
    if row is None:
        return None
    model_value, is_enabled = row
    if not is_enabled:
        return None
    if not model_value:
        return None
    return str(model_value).strip() or None
