...
- resolve_default_model_config_id()     — 返回某类型 is_default 行的 id（供 Corpus 固化）
...
# 硬编码 Embedding 默认 —— 仅在「DB 不可达且未配置任何默认」时的最后回退；
# 权威默认来自 model_configs.is_default（见 _resolve / _resolve_from_default_model_config）。
# 注意：向量列固定为 vector(DEFAULT_EMBEDDING_DIM=1536)，落库模型须输出 1536 维；
# 此 768 维 gemini 占位值无法写入 1536 列，部署可经环境变量覆盖为 1536 维模型。
...
async def resolve_default_model_config_id(model_type: str) -> UUID | None:
    """返回 ``model_configs`` 中 ``is_default=True`` 且 ``enabled=True`` 行的 id；无 / 出错则 None。

    供 Corpus 创建时「固化」当前全局默认模型 —— 将具体 ``model_config.id`` 写入
    ``corpus.config.models.embedding_config_id``，使语料创建即绑定具体模型，免疫日后
    全局默认变更。与 ``_resolve_from_default_model_config`` 共用同一行查询。
    """
    try:
        row = await _load_default_model_config_row(model_type)
    except Exception:
        from negentropy.logging import get_logger

        get_logger("negentropy.config.model_resolver").warning(
            "resolve_default_model_config_id_failed", model_type=model_type, exc_info=True
        )
        return None
...
async def _resolve_from_model_config_row(model_type: str, config_id: UUID | str) -> tuple[str, dict[str, Any]] | None:
    ...
    return await _build_resolved_config(model_type, mc)


async def _build_resolved_config(model_type: str, mc: Any) -> tuple[str, dict[str, Any]]:
    """从已取出的 model_configs 行 + vendor_configs 凭证构建 ``(full_name, kwargs)``。

    供 ``_resolve_from_model_config_row``（按 id）与 ``_resolve_from_default_model_config``
    （按 is_default）复用，避免两处 kwargs 构建逻辑分叉（DRY）。调用方需保证 ``mc`` 已通过
    enabled / 类型校验。
    """
    vendor = mc.vendor
...
async def _load_default_model_config_row(model_type: str) -> Any | None:
    """读取 ``model_configs`` 中 ``is_default=True`` 且 ``enabled=True`` 的单行；无则 None。

    部分唯一索引 ``ix_model_configs_default_unique`` 保证每种 ``model_type`` 至多一行默认，
    故 ``LIMIT 1`` 结果确定。``enabled`` 过滤确保「被设为默认但随后禁用」的行不被选中
    （写端点对 ``is_default`` 与 ``enabled`` 独立更新，二者可能不一致）。
    """
    from sqlalchemy import select
...
async def _resolve_from_default_model_config(model_type: str) -> tuple[str, dict[str, Any]] | None:
    """按 ``model_configs.is_default`` 解析全局默认模型；无默认行返回 None。

    这是 Interface/Model 模型卡片「Default」开关的权威消费点：用户设定的默认模型
    （写入 ``model_configs.is_default``）优先于硬编码 ``_DEFAULT_*_MODEL`` 回退。
    """
    row = await _load_default_model_config_row(model_type)
    if row is None:
        return None
    return await _build_resolved_config(model_type, row)


async def _resolve(model_type: str) -> tuple[str, dict[str, Any]]:
    """核心解析: 缓存 → model_configs.is_default → vendor_configs + 默认模型名 → 硬编码回退。"""
    now = time.monotonic()
    ...
    # 2. model_configs.is_default=True —— 用户在 Interface/Model 设定的全局默认（权威源）
    try:
        result = await _resolve_from_default_model_config(model_type)
        if result is not None:
            name, kwargs = result
            _cache[model_type] = (name, kwargs, now)
            return name, kwargs.copy()
    except Exception:
        # DB 异常（表未迁移 / 不可达）→ 记 warning 并降级到下一层，不冒泡。
        from negentropy.logging import get_logger

        get_logger("negentropy.config.model_resolver").warning(
            "model_resolver_default_row_fallback", model_type=model_type, exc_info=True
        )

    # 3. 默认模型名 + vendor_configs 凭证（无用户默认行时的回退）
    try:
