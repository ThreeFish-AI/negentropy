...
class RoutineEvaluator:
    ...
    async def _judge(self, prompt: str, *, model_override: str | None = None) -> tuple[int, str, str, str, bool | None]:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                score, verdict, reflection, acceptance_met = self._parse(content)
                return score, verdict, reflection, content or "", acceptance_met
            except Exception as exc:
                last_error = exc
    ...
    @staticmethod
    def _parse(content: str | None) -> tuple[int, str, str, bool | None]:
        """解析 judge JSON，做边界裁剪与字段校验。返回 (score, verdict, reflection, acceptance_met)。
        ...
        return score, verdict, reflection, acceptance_met
