 ...
 * 拉取一主五翼 6 Agents 元数据。
 *
 * 通过 next.config rewrites 透明代理到 ui 端 BFF `/api/interface/agents`，
 * 避免在 wiki 重复实现 Agent 字段裁剪与鉴权头转发。
 *
 * 缓存策略：sessionStorage 缓存（每 tab 一份），TTL 由调用方控制。
 * 设计权衡：Agent 列表低频变更（运维事件），用 sessionStorage 而非 SWR /
 * react-query —— wiki 端不引入额外数据层依赖，保持 client bundle 轻量。
 */
import { useCallback, useEffect, useRef, useState } from "react";

export interface AgentSummary {
  /** Agent 名称（如 PerceptionFaculty），用作 forwardedProps.preferred_agent。 */
  name: string;
...
const STORAGE_KEY = "wiki:agent-chat:agents:v1";
...
function normalize(raw: unknown): AgentSummary[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((item): AgentSummary | null => {
      const r = item as RawAgent;
...
function readCache(): AgentSummary[] | null {
...
function writeCache(data: AgentSummary[]): void {
...
export interface UseAgentsResult {
  ...
  rootAgent: AgentSummary | null;
  /** 五翼 Agents（非 root 且 enabled）。 */
  faculties: AgentSummary[];
  loading: boolean;
  error: string | null;
  refresh: () => void;
}
...
export function useAgents(): UseAgentsResult {
  const [agents, setAgents] = useState<AgentSummary[]>(() => readCache() ?? []);
  ...
  const fetchOnce = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/interface/agents", {
        credentials: "include",
        cache: "no-store",
      });
      if (!res.ok) {
