...
构建期 `next build` 读取 `content/` 烘焙为静态 HTML，**不调用主站 API、不读主站数据库**。
详见 [`content/README.md`](./content/README.md)。

> 仓库内 `content/` 是**开发种子 fixture**（首次构建/CI 冒烟用）；真实内容由主站 publish
> 导出——生产/CI 经 `wiki-content-export` workflow 自动提交，**本地**经下方「本地刷新」生成。
> 本地生成的内容请勿提交。

### 本地刷新（publish 后让 wiki 看到新内容）

wiki 不读 DB，故主站「同步并发布」后需把已发布内容**导出**到 `content/` 再重建 wiki：

```bash
# 方式一（推荐）：cli.sh restart 已内置「导出 → 重建 wiki」
./scripts/cli.sh restart

# 方式二：仅导出内容（不重建），再自行构建
./scripts/sync-wiki-content.sh
pnpm --filter negentropy-wiki build
```

前置：postgres 运行 + DB schema 已迁移到当前分支（`uv run alembic upgrade head`）。
`sync-wiki-content.sh` 会把 `NE_SVC_ARTIFACT_BACKEND` 置 `inmemory`（导出工具不用 artifact 后端，
借此容忍用户级 `~/.negentropy/config.yaml` 中可能与本分支枚举不符的取值）。
