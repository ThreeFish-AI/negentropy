...
| 资源文件挂载（Layer 3） | ✓（`scripts/` `references/` `assets/`）        | ✓                                        | ~                                          | ~（**Phase 2**：JSONB `resources` 数组 + `fetch_skill_resource` 路由到 KG/Memory/Knowledge corpus，不直接 fetch URL 防 SSRF；该工具未挂载，见 §3.2 校正） |
