...
| `gen_ai.operation.name`                                                  | `kwargs.litellm_call_type` 或启发式推断          | `chat` / `embeddings`                                                                                   | 从 kwargs 推断，区分 completion 与 embedding        |
...
| `gen_ai.usage.cost`                                                      | LiteLLM cost / 在线价目 / 本地 override          | `0.001234`                                                                                              | **项目自定义属性**，非 OTel GenAI semconv 官方标准；`langfuse.observation.cost_details` 同步对齐 |
