...
PDF 解析、chunk、embedding、KB 写入均**复用现有 `KnowledgeService.ingest_url`**；KG 抽取复用 `AI_PAPER_SCHEMA`（见 `docs/reference/cognizes/engine/036-the-knowledge-graph.md` §Schema 引导构建）。
