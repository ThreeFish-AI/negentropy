...
>
> - 系统全景架构：[docs/framework.md](../architecture/framework.md)
> - Knowledge 模块全景：[docs/knowledges.md](../knowledges.md)
> - Memory 模块设计：[docs/memory.md](../memory/overview.md)
...
**与 Negentropy Memory 模块的互补**：Memory 模块使用 Ebbinghaus 遗忘曲线<sup>[[8]](#ref8)</sup>管理情景记忆（完整公式：`retention_score = min(1.0, e^{-λt} × (1 + ln(1+n)) / 5.0)`，详见 [memory.md](../memory/overview.md)），知识图谱提供**结构化语义记忆**——两者分别对应人类认知的海马体（短期/情景）与新皮层（长期/结构化）。
