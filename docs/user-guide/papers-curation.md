...
进入 `/knowledge/graph`，选择 `agent-papers` Corpus，构建 KG 时指定 `extraction_schema=ai_paper`（参考 [`docs/knowledge-graph/overview.md` §Schema 引导构建](../knowledge-graph/overview.md)）。
