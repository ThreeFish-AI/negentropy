...
> 本文档是 [`memory.md`](./overview.md) 的理论补丁，集中沉淀 Memory 模块的认知科学/信息检索/Agent 系统三类引文，附跨框架对比矩阵。日常使用请直接参考 [`user-guide/memory-basics.md`](../user-guide/memory-basics.md)。
...
> Phase 5 聚焦白皮书既定的四个高/中优先级缺口。所有特性默认关闭、向后兼容、灰度启用，工程契约见 [`memory.md`](./overview.md) §10 与 [`user-guide/memory-basics.md`](../user-guide/memory-basics.md) "高级特性开关"。
