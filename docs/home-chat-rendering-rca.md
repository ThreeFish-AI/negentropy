...
### 3.2 Phase 2 — Hydration invocation_id 映射（根治 ISSUE-041）

`session-hydration.ts:49` 的 `fallbackRunId()` 添加 `invocation_id` 到查找链：

```typescript
function fallbackRunId(payload: AdkEventPayload, sessionId: string): string {
  return payload.runId || payload.invocation_id || payload.threadId || sessionId;
}
```

消除合成 runId 的根因，`isSyntheticRunId()` 逻辑可逐步 deprecated。
