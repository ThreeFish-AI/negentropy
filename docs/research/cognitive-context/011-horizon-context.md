...
>
> 初级 Data Agent 就像一位 **每天都在重新入职、毫无业务常识的天才实习生**：他满腹经纶、数学和逻辑满分，但完全不懂企业的方言黑话与合规底线。
>
> Horizon Context 要做的，是为这位天才实习生配备一套嵌入大厦基座的「受治理带教中枢」（终极入职包），让他秒变懂业务、守规矩的业务老司机：
>
> - 权威业务规章手册：Semantic Views 显式定义（M1）；
> - 内置财务防错算式：查询时动态聚合推导（M2）；
> - 生物识别物理闸机：引擎级原生 RBAC 门禁（M3）；
> - 助教日常偷师与自纠：Cortex Sense 行为挖掘与自纠（M4）；
> - 带教大厅值班老法师：CoCo 混合检索与考证 FAQ（M5）；
> - 案头四维评估尺：四因子信号排序（M6）；
> - 全球工作签证与工业安全插座：Ossie 与 Snowflake 官方 MCP 开放互操作（M7）。
>
...
| **动态现算、保真不走样** | M2 查询时语义正确性        | 发内置防错算式按需现场推导，绝不拿预先汇总的二手死账将就应付                 |
| **原生治理、杜绝穿透**   | M3 引擎原生治理            | 门禁焊在调取底层档案库的唯一必经闸口上，物理封死所有后门，安全锁底层强制生效 |
| **行为挖掘、覆盖长尾**   | M4 双轨富化与自纠          | 手册没写全的暗规则，助教暗中观察老法师用数习惯自动补全，有冲突则鸣笛上报     |
| **标准开放、随处插拔**   | M7 开放互操作              | 配发通用工作护照与工业安全插头，任何外部特聘专家工具插上就能立刻用           |
...
>
> - **阶段一（起草规章）**：起初大厦只有一本冰冷的「机房资产登记簿」（记录底层有哪些账本物理表）；后来为拯救四处碰壁的实习生，编制了第一部装订成册的《业务规章手册》（把含义做成受治理元数据对象）；
> - **阶段二（筑牢闸机与配强助教）**：光有手册不够，大厦直接把生物识别门禁焊进了机房承重墙（引擎原生治理杜绝绕道），同时派出智能助教照着老员工日常用数习惯偷师补全长尾规章（双轨富化）；
> - **阶段三（跨企盟约）**：带教体系走出单一企业，与行业盟友签署通用的国际工作协议（Ossie）并装上标准插座（MCP），让任何外部智囊专家拿着护照都能无缝进驻协同（生态开放）。
...
> [!TIP] **类比**
>
> 过去企业散落的业务口径，就像工位隔板上贴满的私人便利贴与杂乱草稿，写满了硬编码的 SQL 片段，新人看一眼就晕头转向；
>
> M1 机制要做的，就是把这些便利贴全部撕下、分类归档，统一装订成一本**官方核准的《标准业务规章手册》**：
>
> - 明确界定核准账本（TABLES）、勾稽路径（RELATIONSHIPS）、原始项目（FACTS）、切片角度（DIMENSIONS）与官方指标（METRICS）；
> - 每条规章都有明确的责任人和版本号，手册附录里甚至贴心地夹着「实习生使用指南」（AI_SQL_GENERATION）与「前辈考证过的经典 FAQ」（AI_VERIFIED_QUERIES）。
...
> [!TIP] **类比**
>
> 手册里的业务指标不是预先抄死在纸上的固定数字（死报表），而是发给实习生的一套**「内置财务防错逻辑的动态计算套件」**。无论主管需要哪个维度的切片，实习生都拿着机房里最底层的原始单据凭证、按当下的口径现场套用公式重算。
>
> 无论怎么切，这套算式都能自动避开“金额被动翻倍”、“平均数的平均数”或“账户结余跨期累加”等经典陷阱；但若上游交接时早已把原始凭证粉碎、只扔来一张粗暴汇总好的二手旧账（如 dbt 已把明细压缩成日汇总），计算器再聪明也逆向推导不出原始流水。
>
> **这套机制确保「只要底层原始凭证与查询切片合理，实习生按公式现场算绝不会错」，但无法拯救「上游粗暴预处理导致的原料残缺」**。
...
>
> Horizon Context 则是直接把**生物识别闸机焊死在机房唯一的承重墙入口处**——无论是老分析师、BI 软件还是新来的天才实习生，任何人调取数据都必须在数据库引擎底层刷卡验身；被标记为绝密（PRIVATE）的账目连目录都不予展示，彻底封死所有绕道后门。
...
> [!TIP] **类比**
>
> 仅靠资深专家手工编写的规章手册（Semantic Views）权威严谨，但耗时耗力，在庞大的大厦里往往只能覆盖不到 5% 的最核心业务，绝大多数长尾提问在手册里根本翻不到。
>
> 要让带教体系真正接地气，系统配了一位**「智能助教」（Cortex Sense）**：在绝不窥探客户机密隐私（不碰真实数据行）的前提下，助教默默跟在业务老法师们身后，观察他们每天调用的上千条查询习惯和常用报表，把沉睡的暗知识自动提炼补充进手册，补齐剩下的 95%；
>
> 但这位助教有一条**铁打的纪律底线**：当发现专家手册与民间习惯口径打架时，助教**绝对不准自作主张瞎蒙一个**，必须老老实实向业务主管亮起红灯（冲突浮出），交由人工裁定归属。
...
> [!TIP] **类比**
>
> 面对浩瀚的规章手册与海量的历史用数记录，带教大厅的**「金牌值班老法师」**绝不会把整座档案库一股脑砸向实习生——那不仅会瞬间撑爆新人的大脑（上下文窗口超载并诱发严重幻觉），还会浪费极高的沟通与算力成本；
>
> 老法师极其懂分寸：当实习生领着业务问题跑来请教时，老法师迅速从海量规章中撕下最精准的 2~3 页递给新人；更省心的是，如果发现这道题前辈早已核准过标准业务范式（AI_VERIFIED_QUERIES 经典 FAQ），老法师直接调出盖章核准的底稿作答，连现场推算都省了。
...
## 8. M6 · 信号排序：带教案头的四维评估尺，权威压过声量

> [!TIP] **类比**
>
> 老法师案头堆着成百上千条可选的规章条目与历史用数经验，究竟该把哪条指引递给实习生？老法师心里有一套严格的**「四维权衡评估尺」**：
>
> - **契合度（Relevance）**：跟实习生当下的业务提问贴得有多紧；
> - **权威度（Authority）**：是合规委员会盖红章的正式法条，还是民间员工随手记的草稿；
> - **流行度（Popularity）**：在过去数百次实战中是不是大家公认好用的标准套路；
...
>
> - **护照通用（Ossie 开放语义规范）**：规章手册不写成厂商私有的加密暗号，而是采用行业通行的国际标准语言，外部专家一翻即懂；
> - **插座通用（Snowflake 官方 MCP Server）**：在机房前装配标准的工业安全接口，外部专家插上插头就能与大厦数据无缝对话，同时依然全程佩戴安全缰绳，严格受物理承重墙闸机（M3）监管。
