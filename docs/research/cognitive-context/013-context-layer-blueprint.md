...
显式轨道（authority=1.0，人审转 governed）；隐式轨道（<1.0，从日志痕迹拼装解覆盖不动）；eval 自纠环；**冲突契约（核心）**：同名异义→双双 `conflict`→CONFLICT 卡片（无数值、needs_adjudication）→执行层拒绝→人工裁决恢复。行业：DataHub「被看到 50 次的 join 是晋升候选，不是自动答案」；覆盖是动态资产——缺口榜（§11.4）即富化工单队列。边界：富化修**含义**不修数据。
