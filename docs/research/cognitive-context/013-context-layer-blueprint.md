...
瓶颈不在模型，在**含义的供给方式**。同一份数据，销售报 \$14.2M、CFO 报 \$12.8M——Snowflake 官方开场：同一份数据，两个答案。没有人算错数——错的是**语义无人治理**——物理列名如密码（毛收入叫 `amt_ttl_pre_dsc`），口径散落在各报表的 `CASE WHEN` 里，无人裁决「净收入怎么算」。Snowflake 实测：**缺乏语义治理的 agent 裸问企业数据，准确率仅 ~25%（Snowflake 内测）/ 21%（Anthropic 复测）**——缺的不是推理力，是受治理的含义。三个**不可自愈**病灶由此裂出：
