...
你聘请了一位博闻强记的**天才博士（LLM）**。他通读了截至训练数据截止日期（因模型而异，如 GPT-4o 为 2023 年 10 月，Claude 系列则更晚）的大量人类知识，才华横溢却又身处窘境——他被关在一个**没有互联网的房间里**。
...
当用户提问时，管理员先利用 Embedding 在图书馆（向量数据库）中检索出最相关的几页资料，然后把**这些资料连同问题**一起递给博士。博士基于这些新鲜、准确的资料进行即时阅读和回答，从而有效缓解了记忆截止和幻觉等固有缺陷<sup>[[2]](#ref2)</sup>：
...
| **SimHash**           | 余弦相似度     | 适用于文档去重<sup>[[10]](#ref10)</sup>    |
...
**效果**：用不到 1/10 的内存，就能管理 10 倍的数据量，且延迟仍在毫秒级。

**DiskANN** 由微软研究院开发（2019 年发表于 NeurIPS），专为解决 **十亿级向量无法完全载入内存** 的问题而设计<sup>[[16]](#ref16)</sup>。
...
| **内存占用** | 约 32 字节/向量  | PQ 编码，远低于 HNSW |
| **搜索延迟** | 1-5 毫秒          | 95%+ 召回率    |
| **索引构建** | 数小时（十亿级）  | 可并行构建     |
...
**ScaNN (Scalable Nearest Neighbors)** 是 Google Research 提出的高效算法（2020 年发表于 ICML），其杀手锏是 **各向异性向量量化（Anisotropic Vector Quantization）**。
...
- **速度极快**：一眼扫过试卷就能打分（单核 QPS 可达数千），比死抠字眼（计算全几何距离）的传统考官快好几倍。
- **眼光毒辣**：虽然读得快，但对好学生（Top-K）是一个不漏（召回率 95%+）。

根据官方基准测试<sup>[[17]](#ref17)</sup>（注：具体 QPS 数值因硬件配置与参数设置而异，下表为近似参考值）：

| 数据集         | 向量数 | 维度 | QPS（单线程） | 召回率 |
| -------------- | ------ | ---- | ------------- | ------ |
| **SIFT-1M**    | 100 万 | 128  | **7,000-8,000** | 95%    |
| **GIST-1M**    | 100 万 | 960  | **1,500-2,000** | 95%    |
| **GloVe-1.2M** | 120 万 | 100  | **5,000-6,000** | 95%    |
...
| 场景                        | 推荐策略           | 核心理由                                         |
| :-------------------------- | :----------------- | :----------------------------------------------- |
| **大众筛选** (如 `类别=科技`) | **Post-filtering** | 只是排除少量数据，候选集中有足够多的合格结果。     |
...
# 注意：以下为 Weaviate v3 客户端语法，v4 客户端 API 已重构
...
基于公开基准数据（SIFT-1M，128 维，Recall@10=95%）<sup>[[22]](#ref22)</sup>（注：以下数值为近似参考值，实际性能因硬件配置、参数调优与软件版本而异），各路”车队”的成绩单如下：
...
```yaml
# docker-compose.yml
# 注意：以下版本号为写作时最新稳定版，实际使用时请替换为当前最新版本
version: "3.5"
...
# 或（需 CUDA 显卡支持；推荐通过 conda 安装以获得完整 GPU 支持）
conda install -c conda-forge faiss-gpu  # 赛道性能版
```
...
<a id="ref16"></a>[16] S. J. Subramanya, F. Devvrit, R. Kadekodi, R. Krishnaswamy, and H. V. Simhadri, "DiskANN: Fast accurate billion-point nearest neighbor search on a single node," in _Adv. Neural Inf. Process. Syst._, vol. 32, pp. 13771–13781, 2019.
...
<a id="ref18"></a>[18] NVIDIA, "CAGRA: GPU-accelerated graph index for vector search," _NVIDIA RAFT Documentation_, 2024. [Online]. Available: https://docs.rapids.ai/api/raft/stable/
...
<a id="ref24"></a>[24] J. Johnson, M. Douze, and H. Jégou, "Billion-scale similarity search with GPUs," _IEEE Trans. Big Data_, vol. 7, no. 3, pp. 535–547, Sep. 2021.
