...
- **处理方式**：
  1. 新增 `--text-caption`(0.6875rem/11px)、`--text-micro`(0.625rem/10px)，**故意不配对 `--line-height`**——Tailwind v4 仅输出 `font-size`，与裸 `text-[Npx]` 行为逐字节一致，行高沿用 body 无单位 1.375 比值，实现全场景像素零变化（含祖先覆写行高）；
...
- **验证**：残留 `text-[8/9/10/11px]`=0；`text-caption`=198、`text-micro`=267、`tracking-overline`=55 与基线精确对账；编译 CSS 确认仅 `font-size`；chrome_devtools computed-style 探针证 caption/micro 与旧值逐像素一致、overline 字距 +0.1px 子像素；tsc/eslint(`--max-warnings=0`)/727 单测全绿；明暗双模实机抽检 knowledge/base 与 scheduler 渲染正常。
