...
- **处理方式**（权威源切到 routines 表 + 每 tick 校正，详见 [方案 §3.1](pdf-fidelity-patrol-status.md)）：新增 `_reconcile_patrol_status(db)`，每 tick 在 `_collapse_superseded_patrols` 之后运行，以**每 doc 最新的非 cancelled 终态 Routine**（`succeeded`/`failed`，按 `created_at DESC`）为权威重算列——`cancelled` = 被取代/放弃、非真实结论，故排除；`succeeded` 或 `best_score ≥ patrol_qualified_score_threshold` → `done`，否则 `unfixable`；**跳过有 `running`/`paused` Routine 的 doc**（spawn 写的 `in_progress` 是当前真实态，不可回退到旧终态）；幂等（仅变化时写）。迁移 `0093` 用同 SQL 在部署时一次性修复存量受污染数据。dry-run 验证：该 bug doc 的 winner = routine ②（`succeeded`/95）→ 校正为 `done/95`。
