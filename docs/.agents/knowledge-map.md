...
- [精读：Snowflake Horizon Context](../research/cognitive-context/011-horizon-context.md) · [Horizon ↔ negentropy 机制映射](../research/cognitive-context/012-horizon-context-mapping-negentropy.md) — 「住进治理引擎、查询时强制执行」的上下文层七机制 M1–M7（2026-09-17 全局重评选校准：语义视图口径单点×查询期重算[双不变量]/查询期行列级访问策略/语义级治理执行/应答层验证锚定[VQR]/端到端列级血缘/Agent Identity/分类与标签驱动策略传播）+ 富化/检索/互操作三专章保留（0/4 提名降级、各带重评触发器）+ 三阶段演进全景与时间线（语义对象化→治理内嵌→生态开放）+ 关键实证（24.1%→86.3% 双口径全自报、<5% 覆盖率、NDCG 四层增益、477 vs 48 治理≠验证、Looker symmetric aggregates 执行语义反例）+ §16 重评审记录（五判据/四人格盲评/八路对抗验证）；映射 16 条（✅4/🔶8（含部分对齐 1）/⏸4，真增量=verified Q&A/synonyms 字段/冲突裁决面/代理会话权限天花板），M1–M7 七机制各配一张 archify 动效工程图 + §1 病因链对位/§2 组件全景/演进时间线三张总览图 + §10–§12 降级章四张沿用（共十四张，trace 动画）；配套最小原型 [horizon_context_lab.py](../research/cognitive-context/assets/horizon_context_lab.py)（十次破坏性实验）+ [MCP 服务原型](../research/cognitive-context/assets/horizon_context_mcp.py)
- [Context Layer 基础设施设计蓝图](../research/cognitive-context/013-context-layer-blueprint.md) — 由 Horizon Context 精读派生的**通用可复刻**治理上下文层（不绑定本仓内部栈、可独立服务化）：对象存储 / 目录 / 富化自纠 / 治理 / 激活五正交层 + 业界四路线格局对照（平台内嵌/定义即代码/独立可执行层/元数据平面）+ MCP 供给面威胁模型 + known-answer 评测标尺 + 组织与运营对策（Propose→Certify）+ 「治理≠验证」边界对策 + 独立部署演进路线；与 [Context Layer · 上下文治理层技术方案](../concepts/design/context-layer.md) 是「通用蓝图 ↔ negentropy 实例化」的姊妹关系
- [科普视频制作 Pipeline（公共基建，apps/negentropy-influence）](../../apps/negentropy-influence/README.md) — 子项目已从仓库根 media/ 迁入 apps/（分层：pipeline/ 机制 vs episodes/ 内容，路径锚点=根哨兵 `.influence-root`，对目录深度免疫）；全仓可复用的论文→视频九阶段流水线：`pipeline.py` 单入口（status/doctor/build/check/tts/captions/render/qa/all/clean-samples/stages，另有 `--series` 白名单扇出——仅 status/doctor/build/check 可扇出，子命令 flag 逐字转发）+ 每集 `pipeline.toml` 声明式配置（[schema/默认值层](../../apps/negentropy-influence/pipeline/scripts/config.py)：机制常数进代码、toml 只写偏离，缺失会让时长门**点名跳过**而非静默）+ `timing.json` 时序 SSOT（timing.ts 与 Python 共读，双语言镜像漂移结构性消灭）+ [stages.toml](../../apps/negentropy-influence/pipeline/stages.toml) 九阶段唯一声明（序号↔文件号错位 ⑥↔07/⑦↔06 显式化并由 test_stages 执法）；技能规格 skills/01–09，真 Skill 挂载于 [.agent/skills/science-video-pipeline](../../.agent/skills/science-video-pipeline/SKILL.md)（纯路由壳，其死链已入 check_series 受检面）；测试 `pipeline/tests/`（全量 5 秒内跑完，digest 黄金哈希守缓存零失效）；骨架三件套 [templates/video-skeleton](../../apps/negentropy-influence/pipeline/templates/video-skeleton/skeleton.toml)（scaffold 实例化 + verify_skeleton 按系列分组 md5 漂移门，「复制源头」从「任一既有集」收敛为命名模板）；运动层 `video/src/motion/`（frozen，2026-09 重制 EP1 时引入——「共享怎么动、不共享画什么」：令牌/窗口/编排/12 运动模型 hooks + MotionGallery 评审面，铁律与规格见 [skills/06 运动层](../../apps/negentropy-influence/pipeline/skills/06-remotion-implementation.md)；QA 新面：`qa_frames --beat-heads` 入场瞬态补盲 + `--compare` A/B 对拍、`check_script --check-motion` 分镜 @动词 机检）；工具：check_script（分镜覆盖性/时长预算双口径 + `--pre-tts` 合成前置门 + `--pron-candidates` 多音字复听清单）、check_series（系列一致性**六规则**执法 [series.json](../../apps/negentropy-influence/series.json)：口播反串线/多标题顺序/序号绑定/清单完整性含反向登记与撞色/死链/可渲染性）、captions（srt/vtt）、qa_frames --check（黑帧/字幕带侵入/冻帧/WCAG 对比度自动体检）+ `--stills-plan`（分幕复检抽帧计划）、paper_extract（论文取证五子命令）、source_ledger（B 型信源固定提交+双指纹清单，`sync`/`audit` 按系列地图批量建账并离线执法）、refs（样本指纹清单）、tts_progress（长跑热漂移旁路监视）；Stage ① 分两型——A 型论文（paper_extract + paper-notes.md）、B 型文档/代码/课程站点（source_ledger + source-notes.md + 证据三级），见 [skills/01](../../apps/negentropy-influence/pipeline/skills/01-source-extraction.md)。
- [科普视频作品总览（两个系列 8 集）](../../apps/negentropy-influence/series.md) — 机读 SSOT [series.json](../../apps/negentropy-influence/series.json) 顶层为 `seriesList[]`；**自进化系列**（论文型）《上线之后，AI 才开始上学》（金/青/紫，v3 已交付，成片 14:01）→《AI 如何自己变强？》（蓝/橙）→《会写代码的 AI，开始给自己写代码》（绿/洋红）；**Claude Code Harness Engineering**（文档/代码型，5 集 Harness Engineering 改造中，原「通俗全解」版合计 66.9 分钟）《执行层：一个循环，就是全部》（陶土橙/石青/警示红，14:14.73）→《规划层：模型的视野是安排出来的》（鸢紫，13:14.00）→《记忆层：会丢的和不能丢的》（苔绿，13:14.47）→《时机层：谁来按下开始》（霜蓝，13:08.90）→《协作层：从一个到一群》（赭金，13:03.27）；信源 Learn Claude Code s01–s20 双钉（首集钉仓库 main `f9e8b28` 17 章整合版，其余四集钉站点同源修订 `67a9126c` 20 章版，MIT，归属登记见 [source-map](../../apps/negentropy-influence/source-map/claude-code-explained.md)）。多系列执法语义：反串线规则跨系列全局，顺序类规则按系列内判定；口播永不携带集数序号，顺序变更 TTS 代价恒为零。
