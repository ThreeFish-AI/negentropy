...
- [精读：Snowflake Horizon Context](../research/cognitive-context/011-horizon-context.md) · [Horizon ↔ negentropy 机制映射](../research/cognitive-context/012-horizon-context-mapping-negentropy.md) — 「住进治理引擎、查询时强制执行」的上下文层六机制（五段式对象模型/查询时聚合安全/双层 RBAC/显式隐式双轨+冲突浮出/四因子排序/OSI+MCP 互操作）+ 关键实证（24.1%→86.3%、<5% 覆盖率、477 vs 48 治理≠验证）；映射 12 条（✅4/🔶5/⏸3，真增量=verified Q&A/synonyms 字段/冲突裁决面），配套最小原型 [horizon_context_lab.py](../research/cognitive-context/assets/horizon_context_lab.py) + [MCP 服务原型](../research/cognitive-context/assets/horizon_context_mcp.py)
- [Context Layer 基础设施设计蓝图](../research/cognitive-context/013-context-layer-blueprint.md) — 由 Horizon Context 精读派生的**通用可复刻**治理上下文层（不绑定本仓内部栈、可独立服务化）：对象存储 / 目录 / 富化自纠 / 治理 / 激活五正交层 + 「治理≠验证」边界对策 + 独立部署演进路线；与 [Context Layer · 上下文治理层技术方案](../concepts/design/context-layer.md) 是「通用蓝图 ↔ negentropy 实例化」的姊妹关系
