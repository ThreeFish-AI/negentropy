...
- [Mermaid 文本源索引](../assets/mermaid/README.md) — 现役系统文档全部架构图的集中文本源 SSOT 与溯源清单（.mmd → archify HTML → 双主题 PNG 派生链；research / cognizes / 分镜不在此管线）
- [架构图资产库](../assets/architecture/) — 按九分类（core/design/operations/subsystems/user-guide/perceives/wiki/apps/agents）管理的 archify 交互 HTML 与双主题 PNG；concepts 域 80 图已全部替换完成（2026-09-11）
...
- [论文精读：Procedural Graph（arXiv:2609.09153）](../reference/paper-notes/procedural-graphs.md) · [PG ↔ negentropy 机制映射](../reference/paper-notes/pg-mapping-negentropy.md) — 程序性知识外置为带属性有向图（condition/guidance/pitfalls 边）+ 在线邻域定位软指导 + 验证门/拒绝记忆自进化；映射报告对照本仓 routine/巡检/evolution 五条机制（M1 门控平局、M3 报告纪律已对齐，M2 拒绝记忆进进化回路建议落地，M4 边属性 YAGNI 暂缓），配套最小原型 [pg_lab.py](../reference/paper-notes/assets/pg_lab.py)
- [精读：Snowflake Horizon Context](../reference/paper-notes/horizon-context.md) · [Horizon ↔ negentropy 机制映射](../reference/paper-notes/horizon-context-mapping-negentropy.md) — 「住进治理引擎、查询时强制执行」的上下文层六机制（五段式对象模型/查询时聚合安全/双层 RBAC/显式隐式双轨+冲突浮出/四因子排序/OSI+MCP 互操作）+ 关键实证（24.1%→86.3%、<5% 覆盖率、477 vs 48 治理≠验证）；映射 12 条（✅4/🔶5/⏸3，真增量=verified Q&A/synonyms 字段/冲突裁决面），配套最小原型 [horizon_context_lab.py](../reference/paper-notes/assets/horizon_context_lab.py) + [MCP 服务原型](../reference/paper-notes/assets/horizon_context_mcp.py)
