...
- [精读：Snowflake Horizon Context](../research/cognitive-context/011-horizon-context.md) · [Horizon ↔ negentropy 机制映射](../research/cognitive-context/012-horizon-context-mapping-negentropy.md) — 「住进治理引擎、查询时强制执行」的上下文层七机制 M1–M7（五段式对象模型/查询时聚合安全+按需物化/引擎原生治理/双轨富化与自纠[Autopilot+Cortex Sense]/检索激活/四因子信号排序/OSI+MCP 互操作）+ 三阶段演进全景与时间线（语义对象化→治理内嵌→生态开放）+ 关键实证（24.1%→86.3%、83/47/23 双口径、<5% 覆盖率、NDCG 四层增益、477 vs 48 治理≠验证）；映射 12 条（✅4/🔶5/⏸3，真增量=verified Q&A/synonyms 字段/冲突裁决面），M1–M7 七机制各配一张 archify 动效工程图 + §1 病因链与机制对位 + §2 组件全景/演进时间线两张总览图共十张（trace 动画）；配套最小原型 [horizon_context_lab.py](../research/cognitive-context/assets/horizon_context_lab.py) + [MCP 服务原型](../research/cognitive-context/assets/horizon_context_mcp.py)
