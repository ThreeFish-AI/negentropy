...
- [Mermaid 文本源索引](../assets/mermaid/README.md) — 现役系统文档全部架构图的集中文本源 SSOT 与溯源清单（.mmd → archify HTML → 双主题 PNG 派生链；范围按文档界定——已含 research/cognitive-context 的精读与蓝图，其余 research / cognizes / 分镜不在此管线）
- [架构图资产库](../assets/architecture/) — 按十分类（core/design/operations/subsystems/user-guide/perceives/wiki/apps/agents/cognitive-context）管理的 archify 交互 HTML 与双主题 PNG；concepts 域 80 图已全部替换完成（2026-09-11）
- [Framework（系统框架）](../concepts/framework.md) — 架构设计的权威参考：三层架构视图（§2.1，正文为高清静图）、应用边界与技术栈（§2.2）、一核五翼编排、三条标准流水线、启动引导、可观测性、DB schema 分域、测试金字塔与 CI/CD，11 张图正交覆盖各维度（含 §6.5 引擎内景：调度/Routine/Claude Code/Definitions/Patrol/Evolution 七子系统）；各图的文本源集中于 [Mermaid 文本源索引](../assets/mermaid/README.md)
- [Architecture Diagram（交互式架构图）](../assets/architecture/core/architecture-diagram.html) — 三层拓扑交互 HTML（archify 生成，英文；11 处仓库源码锚点、4 章 13 停引导叙事、明暗双主题）：wiki 纯静态导出、perceives MCP 服务、PG17 等校准事实的 SSOT 可视化。**唯一可编辑源是 [`core/negentropy-architecture.mmd`](../assets/mermaid/core/negentropy-architecture.mmd)（Mermaid 文本源 SSOT）**，本 HTML 与 [架构图媒体资产](../assets/architecture/core/) 均为其派生产物；消费点三处——[README](../../README.md) §Three-Tier Architecture / [中文 README](../i18n/zh-CN/README.md) §三层架构（均以 GIF 动图为主视觉）/ framework.md §2.1（暗色高清静图内嵌 + 文本源）。改图须「先改 Mermaid → 用 archify 重新生成交互 HTML（整体替换）→ 跑采集脚本派生媒体产物 → 三处消费点同步核对」，严禁手改生成物 HTML
- [架构图媒体资产](../assets/architecture/core/) — 同一张三层拓扑的四类派生产物：[暗色](../assets/architecture/core/negentropy-architecture-dark.png) / [亮色](../assets/architecture/core/negentropy-architecture-light.png) 5120×2880 高清 PNG · [双主题矢量 SVG](../assets/architecture/core/negentropy-architecture.svg) · [29 秒动效 MP4](../assets/architecture/core/negentropy-architecture-story.mp4)（1280×720）· [同源 GIF](../assets/architecture/core/negentropy-architecture-story.gif)（720×406，README 内唯一能就地播放的形态）。由 [`scripts/capture-arch-media.mjs`](../../scripts/capture-arch-media.mjs) 可复现生成；落盘位置、体积门与两条渲染链路约束见 [文档媒体资产规范](./doc-media-assets.md)
...
- [论文精读：Procedural Graph（arXiv:2609.09153）](../research/self-evolution/142-procedural-graphs.md) · [PG ↔ negentropy 机制映射](../research/self-evolution/143-pg-mapping-negentropy.md) — 程序性知识外置为带属性有向图（condition/guidance/pitfalls 边）+ 在线邻域定位软指导 + 验证门/拒绝记忆自进化；映射报告对照本仓 routine/巡检/evolution 五条机制（M1 门控平局、M3 报告纪律已对齐，M2 拒绝记忆进进化回路建议落地，M4 边属性 YAGNI 暂缓），配套最小原型 [pg_lab.py](../research/self-evolution/assets/pg_lab.py)
- [论文精读：Dream-RSI（arXiv:2609.14858）](../research/self-evolution/144-dream-rsi.md) · [Dream-RSI ↔ negentropy 机制映射](../research/self-evolution/145-dream-rsi-mapping-negentropy.md) — 元探索层的递归自我改进：把累积发现历史（发现树）当**重放模拟器**、离线「做梦」（确定性重放：非根→唯一记录子节点/根→最早未揭示）近零成本评估探索策略、重放目标 V 三分量（质量−成本+并行度）、argmax 候选含当前策略防回退（V\*≥V⁰）、只改策略代码不动底座；实证含 162×/50× 调用节省与 §5.1 语义引导 over-constrain 消融；笔记含两相循环/重放模拟器两张 archify 管线图 + 费曼考评对抗实录（三维盲答→诊断→变式复考）与批判性边界五条（重放-在线相关性零消融/做梦成本不计账/正文-附录评估口径分裂等）；映射六条对照本仓（M3 候选包含保证与 M5 策略层冻结已同构、M2 Judge 锚定同向、M1 重放模拟器 YAGNI 暂缓并绑定三触发条件、M6 对 Reflexion/reflections/patrol 注入的 over-constrain 边界警示），配套最小原型 [dream_rsi_lab.py](../research/self-evolution/assets/dream_rsi_lab.py)（五次破坏性实验：拔候选保证/拔 β₂/拔 β₁/重放越权排名反转/语义引导锁死）
- [精读：Snowflake Horizon Context](../research/cognitive-context/011-horizon-context.md) · [Horizon ↔ negentropy 机制映射](../research/cognitive-context/012-horizon-context-mapping-negentropy.md) — 「住进治理引擎、查询时强制执行」的上下文层七机制 M1–M7（2026-09-17 全局重评选校准：语义视图口径单点×查询期重算[双不变量]/查询期行列级访问策略/语义级治理执行/应答层验证锚定[VQR]/端到端列级血缘/Agent Identity/分类与标签驱动策略传播）+ 富化/检索/互操作三专章保留（0/4 提名降级、各带重评触发器）+ 三阶段演进全景与时间线（语义对象化→治理内嵌→生态开放）+ 关键实证（24.1%→86.3% 双口径全自报、<5% 覆盖率、NDCG 四层增益、477 vs 48 治理≠验证、Looker symmetric aggregates 执行语义反例）；映射 16 条（✅4/🔶8（含部分对齐 1）/⏸4，真增量=verified Q&A/synonyms 字段/冲突裁决面/代理会话权限天花板），M1–M7 七机制各配一张 archify 动效工程图 + §1 病因链对位/§4 组件全景/演进时间线三张总览图 + §3 三阶段治理骨架图（能力 × 成熟度 × 承重/降级锚点）+ §10–§12 降级章四张沿用（共十五张，部分带 trace 动效）；配套最小原型 [horizon_context_lab.py](../research/cognitive-context/assets/horizon_context_lab.py)（十次破坏性实验）+ [MCP 服务原型](../research/cognitive-context/assets/horizon_context_mcp.py)
- [Context Layer 技术蓝图与方案](../research/cognitive-context/013-context-layer-blueprint.md) — **精炼设计蓝图 SSOT（2026-09-22 深度清减 3.8W→1W 字级；同日内容面重铸为视频前置内容载体——去电报化·三拍叙事·比喻单射·术语首释，现约 1.03W 字）**：五正交层（对象/目录/富化/治理/激活）机制本质（M1–M7）+ 通用设计规格（对象模型/三视图信任归一/冲突契约/双层防线/resolve 契约与互操作三重边界）+ 业界四路线格局 + MCP 供给面威胁模型 + known-answer 评测标尺 + negentropy 实例化总装（ADR-1/2/3、9 路来源/三缺口/五子系统契约/16 条映射状态表 ✅/🔶/⏸、Phase 1–3 施工明细）+ 双轨演进（P0–P3 × Phase 1–3）+ 七条批判边界与重评触发器；全量机制载荷/组件全景/时间线/实证由 [011 精读笔记](../research/cognitive-context/011-horizon-context.md)（精读终稿、冻结）承载，共用参考亦在 011（重评选过程性审计已于 2026-09-22 裁撤、存 git 史）（原 `docs/concepts/design/context-layer.md` 已于 2026-09-21 并入并删除）
- [Learn Claude Code 五层 Harness 精读（总览 + 五章）](../research/agent-harness/170-claude-code-harness-overview.md) · [① 工具与执行](../research/agent-harness/171-claude-code-tooling-execution.md) · [② 规划与协调](../research/agent-harness/172-claude-code-planning-coordination.md) · [③ 记忆管理](../research/agent-harness/173-claude-code-memory-management.md) · [④ 并发与时机](../research/agent-harness/174-claude-code-concurrency.md) · [⑤ 多 Agent 平台](../research/agent-harness/175-claude-code-multi-agent-platform.md) — 以课程站点修订与 [shareAI-lab/learn-claude-code](https://github.com/shareAI-lab/learn-claude-code) 仓库 main 整合版（MIT，钉 `f9e8b280`）双轨一手材料为信源的**跨层综观**：五层依赖链（每层的存在理由写在上一层的失败里）+ **角色台账单射贯穿**（工坊剧场，含三条防撞铁律）+ 证据三级纪律（闭源分析必带归属句、不转引行号）+ **main 轨视频未取材两章的原生精读**（`s16_workflow_runtime` 全仓唯一的事件循环扇出并发：`parallel` 有屏障 vs `pipeline` 无屏障、journal 幂等续跑、双重熔断；`s17_goal_loop` 无工具评估器 + 停止钩子决策机 + 显式防注入——**该两节本组是唯一事实源**，自带固定提交与实测行数）+ 跨章级文档—实现不一致清单（README 声称并发实为串行 / 站点跳过环检测而仓库实现有 / 全仓无流式 / 无 CLAUDE.md 加载机制以自研记忆目录替代 / 无 Grep 工具）+ **仓内旧 12 课轨与根级 17 课轨章号不对应的导航警示**（本仓 [025 记忆系统](../concepts/subsystems/025-the-memory-system.md) §2.5 与 `context_assembler.py` 两处既有引用即引了旧轨编号）+ 与 [038 Claude Code 集成](../concepts/subsystems/038-claude-code-integration.md) / [039 Routine](../concepts/subsystems/039-the-routine-system.md) / [040 多 Agent 学部](../concepts/subsystems/040-routine-multi-agent-faculty.md) / [Skills](../concepts/design/skills.md) 的机制对位；配套最小原型 [cc_harness_lab.py](../research/agent-harness/assets/cc_harness_lab.py)（纯标准库、确定性、`--selftest` 秒级）含**六次破坏性实验**，其中 D2/D6 补上了材料「只有论证没有实验」的缺口并得出比材料更细的结论——压缩顺序与「未读结果不许动」是**两道独立保险**，单拆其一无损失、同拆才出现原文永久丢失，另暴露占位符不幂等致指针二次丢失。**SSOT 边界**：逐章引语/可调参数/生产版行号/分歧清单归各集 source-notes，章→集归属与钉归 [系列信源地图](../../apps/negentropy-influence/source-map/claude-code-explained.md)，本组只链接不重述，冲突以二者为准
- [科普视频内容工作区（apps/negentropy-influence）+ to-video 技能](../../apps/negentropy-influence/README.md) — **机制外置 skill、本仓持内容与发布顺序 SSOT**：论文→视频九阶段流水线机制（`pipeline.py` 单入口、[stages.toml](https://github.com/ThreeFish-AI/to-video/blob/main/pipeline/stages.toml) 九阶段唯一声明、[config.py](https://github.com/ThreeFish-AI/to-video/blob/main/pipeline/scripts/config.py) schema/默认值层、`timing.json` 时序 SSOT、skills/01–09 规格、[骨架模板三件套](https://github.com/ThreeFish-AI/to-video/blob/main/pipeline/templates/video-skeleton/skeleton.toml)、`pipeline/tests`）已迁出本仓，收敛为公开技能 [to-video](https://github.com/ThreeFish-AI/to-video)（GitHub ThreeFish-AI/to-video，MIT，技能入口 [SKILL.md](https://github.com/ThreeFish-AI/to-video/blob/main/SKILL.md)，安装于 `~/.claude/skills/to-video`；运动层铁律与规格见 [skills/06](https://github.com/ThreeFish-AI/to-video/blob/main/pipeline/skills/06-remotion-implementation.md)、信源 A/B 两型见 [skills/01](https://github.com/ThreeFish-AI/to-video/blob/main/pipeline/skills/01-source-extraction.md)）——机制文档以 skill 仓为唯一事实源，本仓零副本（集侧 scripts/ 均为转发 skill 解析器的薄包装）。本仓持有：episodes/ 分集 + [series.json](../../apps/negentropy-influence/series.json)（发布顺序 SSOT；check_series 六规则执法——口播反串线/多标题顺序/序号绑定/清单完整性含反向登记与撞色/死链/可渲染性）+ source-map/ 信源地图 + voices/ 参考音频，路径锚点=根哨兵 `.influence-root`（skill 兼容识别，对目录深度免疫）；check_series 的工程级受检面（规则 2/3/5 扫描本文件与 CHANGELOG）由工作区根 to-video.toml 的 `[check_series] project_globs` 声明。
- [科普视频作品总览（三个系列 11 集）](../../apps/negentropy-influence/series.md) — 机读 SSOT [series.json](../../apps/negentropy-influence/series.json) 顶层为 `seriesList[]`；**自进化系列**（论文型）《上线之后，AI 才开始上学》（金/青/紫，v3 已交付，成片 14:01）→《AI 如何自己变强？》（蓝/橙）→《会写代码的 AI，开始给自己写代码》（绿/洋红）；**Claude Code Harness Engineering**（文档/代码型，5 集 Harness Engineering 改造中，原「通俗全解」版合计 66.9 分钟）《执行层：一个循环，就是全部》（陶土橙/石青/警示红，14:14.73）→《翻旧账不花钱：AI 在梦里改章程》（冰蓝/暖白/嫩绿，v1 终渲待审 10:30.87，archify 混合 16 图首集）→《规划层：模型的视野是安排出来的》（鸢紫，13:14.00）→《记忆层：会丢的和不能丢的》（苔绿，13:14.47）→《时机层：谁来按下开始》（霜蓝，13:08.90）→《协作层：从一个到一群》（赭金，13:03.27）；信源 Learn Claude Code s01–s20 双钉（首集钉仓库 main `f9e8b28` 17 章整合版，其余四集钉站点同源修订 `67a9126c` 20 章版，MIT，归属登记见 [source-map](../../apps/negentropy-influence/source-map/claude-code-explained.md)）。多系列执法语义：反串线规则跨系列全局，顺序类规则按系列内判定；口播永不携带集数序号，顺序变更 TTS 代价恒为零。
