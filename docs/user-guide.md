...
| 拖拽节点 | 按住节点拖动调整布局 |
| 缩放/平移 | 鼠标滚轮缩放，拖动画布平移 |
| 点击节点 | 右侧面板显示实体详情（名称、类型、ID） |
| 搜索实体 | 搜索栏输入关键词，Enter 执行混合检索 |

节点颜色按实体类型区分：蓝色（person）、绿色（organization）、黄色（location）、红色（event）、紫色（concept）等。
