...
4. [知识库管理](#4-知识库管理)
5. [记忆系统使用](#5-记忆系统使用)
   - [5.8 记忆形成机制](#58-记忆形成机制)
   - [5.9 理解保留分数](#59-理解保留分数)
   - [5.10 搜索最佳实践](#510-搜索最佳实践)
   - [5.11 自动化配置指南](#511-自动化配置指南)
   - [5.12 故障排除](#512-故障排除)
...
> 📚 **理论与业界对标**：参见 [conversation-foundation.md](./architecture/conversation-foundation.md)。
...
#### 4.5.7 全局问答（GraphRAG Global Search）
...
> 更深入的知识系统设计，请参阅 [知识系统](./knowledges.md) 和 [知识图谱](./knowledge-graph/overview.md)。
...
> - 📚 **架构/理论** → [`memory.md`](./memory/overview.md) + [`memory-whitepaper.md`](./memory/whitepaper.md)
>
...
> 更深入的记忆系统设计，请参阅 [记忆系统](./memory/overview.md)。

### 5.8 记忆形成机制

**何时触发**：每次对话会话结束（`add_session_to_memory`）时自动触发记忆巩固。

**巩固管线三阶段**：

```mermaid
flowchart LR
    S["对话事件"] --> Seg["分段<br/>按 speaker turn 分组"]
    Seg --> Ori["去重检测<br/>cosine similarity ≥ 0.9"]
    Ori --> |"新记忆"| Store["存储<br/>附带初始 retention_score"]
    Ori --> |"重复"| Skip["跳过"]

    style S fill:#3B82F6,stroke:#1E3A8A,color:#FFF
    style Store fill:#10B981,stroke:#065F46,color:#FFF
    style Skip fill:#9CA3AF,stroke:#4B5563,color:#FFF
```

1. **分段**：对话按每 5 个 speaker turn 为一组拆分为多段（user+model 配对），每段独立存储为一条记忆
2. **去重**：对每段生成 embedding 向量，与现有记忆比对。语义相似度 ≥ 0.9 的段落跳过
3. **存储**：新记忆附带初始 `retention_score`（基于内容长度和信息密度启发式计算）

**事实自动提取**：对话中识别到用户偏好、个人信息、规则指令时，自动提取为结构化事实（Facts），无需手动创建。

### 5.9 理解保留分数

保留分数（Retention Score）基于**艾宾浩斯遗忘曲线**的指数衰减模型：

```
retention_score = min(1.0, e^(-λ × 天数) × 频率增强 / 5.0)
频率增强 = 1 + ln(1 + 访问次数)
```
...
|:--|:--|:--|
| ≥ 0.5 | 近期被访问，信息鲜活 | 🟢 绿色 |
| 0.1 ~ 0.5 | 逐渐衰减，仍有参考价值 | 🟠 橙色 |
| < 0.1 | 即将被自动清理 | 🔴 红色 + 警告 |

**影响衰减的因素**：
- **时间**：距最后访问天数越多，衰减越大（λ=0.1 时约 7 天降至 50%）
- **频率**：被搜索召回的记忆会自动增加 `access_count`，频率增强减缓衰减
- **配置**：Automation 页面可调整 `decay_lambda`（越大衰减越快）
...
| 优先级 | 策略 | 说明 |
|:--|:--|:--|
| 1 | Hybrid Search | 语义 + BM25 融合（最精准） |
| 2 | Vector Search | 纯语义相似度 |
| 3 | BM25 Keyword | 全文关键词匹配 |
| 4 | ILIKE Fallback | 模糊模式匹配 |

**搜索技巧**：
- 使用自然语言描述（如"上周讨论的部署方案"）比关键词堆砌效果更好
- 支持按记忆类型过滤（`memory_type`）和日期范围（`date_from` / `date_to`）
- 支持分页（`limit` + `offset`）

### 5.11 自动化配置指南

进入 **Memory / Automation** 页面配置记忆系统的自动化行为。以下是三种典型场景的推荐配置：

**高频对话型**（日常频繁使用、大量短对话）：

| 参数 | 推荐值 | 说明 |
|:--|:--|:--|
| `decay_lambda` | 0.15 | 较快衰减，避免记忆膨胀 |
| `low_retention_threshold` | 0.15 | 适度清理阈值 |
| `auto_cleanup_enabled` | true | 开启自动清理 |
| `cleanup_schedule` | `0 3 * * *` | 每天凌晨 3 点清理 |
| `max_tokens` | 3000 | 适中的上下文预算 |

**低频知识型**（偶尔使用、但每次深度对话）：

| 参数 | 推荐值 | 说明 |
|:--|:--|:--|
| `decay_lambda` | 0.05 | 缓慢衰减，保留长期知识 |
| `low_retention_threshold` | 0.05 | 较低清理阈值 |
| `auto_cleanup_enabled` | true | 开启自动清理 |
| `cleanup_schedule` | `0 2 * * 0` | 每周日凌晨 2 点清理 |
| `max_tokens` | 6000 | 较大的上下文预算 |

**合规严格型**（需要审计追踪的场景）：

| 参数 | 推荐值 | 说明 |
|:--|:--|:--|
| `decay_lambda` | 0.1 | 默认衰减速率 |
| `min_age_days` | 30 | 至少保留 30 天 |
| `auto_cleanup_enabled` | false | 手动审计后再清理 |
| `consolidation.enabled` | true | 开启定期巩固 |

### 5.12 故障排除

| 问题 | 可能原因 | 解决方案 |
|:--|:--|:--|
| 搜索无结果 | 记忆未生成 / embedding 失败 | 检查 Dashboard 是否有记忆条目；检查 embedding 配置 |
| Dashboard 数据不更新 | 浏览器缓存 | 刷新页面或清除浏览器缓存 |
| pg_cron 不可用 | 数据库未安装扩展 | 系统自动降级为应用层调度器；不影响核心记忆功能 |
| Facts 重复 | 相同 key 的多次 upsert | 系统使用唯一约束自动合并，最新值覆盖旧值 |
| 保留分数异常 | `decay_lambda` 配置不当 | 在 Automation 页面调整衰减参数 |
| 记忆未从对话提取 | 巩固管线未触发 | 确认 `memory_backend` 配置为 `postgres` 而非 `inmemory` |

### 5.13 理解记忆重要性

每条记忆和事实都有一个 **重要性评分**（Importance Score），范围 0.0 ~ 1.0，反映该条目的固有价值。

**影响因素**：

| 因素 | 说明 |
|:--|:--|
| 记忆类型 | 偏好类 > 流程类 > 事实类 > 情景类 |
| 访问频率 | 被频繁召回的记忆重要性更高 |
| 关联事实 | 关联的事实数量越多越重要 |
| 创建时间 | 近期创建的记忆获得额外加分 |

**在 Dashboard 中的体现**：仪表盘新增 `Avg Importance` 和 `High Importance` 指标，展示所有记忆的平均重要性和高重要性条目数量。

### 5.14 处理冲突记忆

当新提取的事实与已有事实冲突时（例如用户更新了偏好），系统自动检测并记录冲突：

**自动冲突处理**：

1. **检测**：Fact upsert 时，自动检测同 key 不同 value 的情况
2. **分类**：自动判断为矛盾（contradiction）、时间更新（temporal_update）或细化（refinement）
3. **解决**：大多数情况自动 supersede（旧事实标记为已取代），少数保留两者

**管理员操作**（通过 API）：

| 端点 | 功能 |
|:--|:--|
| `GET /memory/conflicts` | 查看冲突列表 |
| `POST /memory/conflicts/{id}/resolve` | 手动解决冲突 |
| `GET /memory/facts/{id}/history` | 查看事实的版本演变链 |

### 5.15 自动记忆注入

系统在创建新对话会话时，会**主动注入**与当前用户最相关的记忆，减少「冷启动」信息缺失。

**复合评分排序**：系统按以下权重选择最相关的记忆注入上下文：
...
**缓存机制**：预加载结果缓存 1 小时，记忆巩固或事实更新后自动刷新。

**手动触发**：通过 `POST /memory/proactive/{user_id}` 可手动触发重新计算。

### 5.16 探索记忆关联

记忆之间可以建立多种关联关系，支持多跳扩展检索：

**关联类型**：

| 类型 | 说明 | 自动发现 |
|:--|:--|:--|
| semantic | 语义相似的记忆 | 是（相似度 > 0.75） |
| temporal | 时间邻近的记忆 | 是（30 分钟窗口） |
| thread_shared | 同一会话产生的记忆 | 是（共享 thread_id） |
| manual | 用户/管理员手动创建 | 否 |

**多跳搜索**：从一条记忆出发，沿关联链扩展最多 3 跳，找到更多相关记忆。只有权重大于 0.6 的强关联才会被遍历。

**管理操作**（通过 API）：

| 端点 | 功能 |
|:--|:--|
| `GET /memory/{id}/associations` | 查看记忆的关联列表 |
| `POST /memory/associations` | 创建手动关联 |
| `DELETE /memory/associations/{id}` | 删除关联 |
...
> 完整的 Wiki 运维文档，请参阅 [Wiki 运维指引](./wiki/ops.md)。
...
| [记忆系统](./memory/overview.md)                    | `docs/memory/overview.md`                      | 记忆生命周期与治理机制             |
| [知识图谱](./knowledge-graph/overview.md)           | `docs/knowledge-graph/overview.md`             | 图建模与查询实现                   |
| [SSO 集成](./sso.md)                       | `docs/sso.md`                         | Google OAuth 认证配置              |
| [QA 流水线](./qa-delivery-pipeline.md)     | `docs/qa-delivery-pipeline.md`        | 质量门禁与发布流程                 |
| [Wiki 运维](./wiki/ops.md)      | `docs/wiki/ops.md`         | Wiki 站点的部署与运维              |
