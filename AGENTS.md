...
- **Operational Excellence (卓越运营)**:
  ...
  4. **Git Commit**: 在需要提交变更到 Git 时，一律使用 Claude Code 的自定义 Slash Command: `/commit-no-push` 进行 git commit 操作（若非 Claude Code 运行环境，则直接读取 /commit-no-push 命令中的规则，按照其中的规则进行 git commit 操作）。不要执行 Rebase。
