...
const VennStage: React.FC<{step: 0 | 1 | 2 | 3}> = ({step}) => {
  ...
  const overlap = interpolate(step, [1, 2], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
      ...
      <div style={{position: 'relative', width: 1280, height: 720}}>
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            paddingTop: 90,
            transform: `translateX(${-overlap * 120}px)`,
          }}
        >
          <div style={{fontFamily: theme.sans, fontSize: 38, fontWeight: 700, color: theme.code}}>编码智能体</div>
          {litLeft && (
            <div style={{marginTop: 26, display: 'flex', flexDirection: 'column', gap: 12, fontFamily: theme.sans, fontSize: 26, color: theme.text}}>
...
const ThreeNutrients: React.FC = () => {
  ...
  const phase = Math.floor(frame / 46); // 三段：反馈→上下文→轨迹（每段~46帧）
  return (
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
                ...
                {[0, 1, 2, 3].map((r) => (
                  <div
                    ...
                    style={{
                      ...
                      width: `${100 - r * 18 - i * 4}%`,
                      opacity: interpolate(frame, [i * 6 + r * 5, i * 6 + r * 5 + 8], [0, 0.9], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}),
                    }}
                  />
                ))}
              </div>
              ...
              {['✓ 成功', '✗ 失败', '✓ 成功', '✗ 失败', '✓ 成功'].map((s, i) => (
                <div
                  ...
                  style={{
                    ...
                    opacity: interpolate(frame, [i * 6, i * 6 + 8], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}),
                  }}
                >
                  {s}
                </div>
...
export const P1Boundary: React.FC<{scene: SceneRange}> = ({scene}) => {
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence {...w('p1-14', 'p1-22')} name="1-F 三大养料">
        <ThreeNutrients />
      </Sequence>
