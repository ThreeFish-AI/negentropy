...
| 7 | `length_penalty>0`（长句吞尾） | 机制明确（§4.2），但**本轮未复现收益** | 进缓存摘要 ⇒ 改档即重录 | #6 已解除阻塞：按 §6.5 协议跑；固定 seed + n≥20 长句，判据=尾部字词完整率（非墙钟） | ⬜ **待验证（已可做）** |
| 8 | `repetition_penalty` 定向扫描 | 上游自 v1 沿用 10.0 且无任何测试支撑（§4.3） | 同上；且**与音色耦合**不可迁移 | #6 已解除阻塞：固定 seed + beams=1，30 句覆盖长/短/数字/英文 | ⬜ **待验证（已可做）** |
