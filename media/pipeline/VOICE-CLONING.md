...
- 每句墙钟与文本长度及束宽相关：MPS fp32 实测 RTF≈40–58（3 束，即 `sunny-steady`）/ ≈12–14（1 束，如 `sunny`）。**1 束整集（约 180–230 句）约 2.5–3.5 小时；3 束按口径要 8–10 小时**——请据此选档并 `nohup` 挂后台跑，按句缓存可断点续跑（见 §六）；
